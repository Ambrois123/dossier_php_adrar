...
try{
    $pdo = new PDO ('mysql:host=localhost', 'formation_adrar', 'formation_adrar');

    $pdo->setAttribute(PDO::ERRMODE_EXCEPTION, PDO::ATTR_ERRMODE);

    if($pdo->exec('CREATE DATABASE restaurant_server') !== false){

        $serveur = new PDO ('mysql:host=localhost;dbname=restaurant_server', 'formation_adrar', 'formation_adrar');

            if($serveur->exec('CREATE TABLE users(
                user_id INT not null PRIMARY KEY AUTO_INCREMENT,
                user_email text not null,
                user_role ENUM("visiteur", "client", "admin"),
